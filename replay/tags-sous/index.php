          ...
          class="table table-bordered table-striped">
            ...
            <tbody>       
              ...
              <tr class="table table-bordered">
                  ...
                  <td>
                    <a title="Voir la vidéo sur Replay" target="_blank" href="https://www.replay.gp/@<?php echo($col['username']); ?>">
                      <?php echo($col['mot_french']); ?>
                    </a>
                  </td>

                  <td>
                    <a title="Le N°<?php echo($col['id']); ?>, correspond a la categorie n°<?php echo($col['categorie']); ?>" href="index.php?nav=2&q=<?php echo($col['categorie']); ?>&action=rechercher">
                      
                    <?php
                      // Afficher les données de la table : langs (continent)
                      $cat = $db->query('SELECT * FROM tags_categorie WHERE id="'.$col['categorie'].'"');
                      while($result_cat=$cat->fetch()){
                    ?> 
                    <?php echo($result_cat['categorie_french']); ?>
                    <?php } ?>
